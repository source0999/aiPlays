...
def manual(config: AppConfig, verify_ram: bool = False) -> int:
    env = PyBoyEnv(config, render_mode="human")
    try:
        observation, info = env.reset(seed=config.training.seed)
        print(
            "Window open. PyBoy SDL2 receives its usual keyboard controls. Close its window or Ctrl+C to exit."
        )
        while True:
